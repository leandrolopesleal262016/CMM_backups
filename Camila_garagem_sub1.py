...
##        s.connect ((host,port))
##
##        command = ("7000 185920E60700006")  # Modo teste de funcionamento
##        s.send(str.encode(command))
##        reply = s.recv(1024)
##        print(reply.decode('utf-8'))
##        
...
##            bar3 = 1
##
##        if (chave_mudanca == 1 and bar3 == 1):
##
##            time.sleep(0.1)
##
##        if (chave_mudanca == 0 and bar3 == 1):
##
##            bar3 = 0
##            print("Chave de mudança desligada" )
##            pygame.mixer.music.load('mp3/151.mp3')  # Chave de mudança desligada 
##            pygame.mixer.music.play()
##            while pygame.mixer.music.get_busy():
##                time.sleep(0.1)
##            
...
##            time.sleep(0.1)
##
##        if (garagem_aberto == 0 and ponto == 1): # Ponto magnético aberto
##            
...
##                s.connect ((host,port))
##
##                command = ("7000 185920R60700006")  # Modo teste de funcionamento
##                s.send(str.encode(command))
##                reply = s.recv(1024)
##                print(reply.decode('utf-8'))
##                
  ...
  def mede_tensao_baterias():
    ...
    if v1<16:
      ...
      try:
        ...
        command = ("7000 185920E30200006")  # Bateria do sistema está baixa
        s.send(str.encode(command))
        reply = s.recv(1024)
        print(reply.decode('utf-8'))
    ...
    if v1>16 and bat == 0:
        ...
        try:
            ...
            command = ("7000 185920R30200006")  # Bateria do sistema está ok
            s.send(str.encode(command))
            reply = s.recv(1024)
            print(reply.decode('utf-8'))
...
##    print("aux2 =",aux2)
    ...
    if (temperatura >= 75):
      ...
      try:
          ...
          command = ("7000 185920E15800006")  # Alta temperatura no processador do SEA (setor 18)
          s.send(str.encode(command))
          reply = s.recv(1024)
          print(reply.decode('utf-8'))
...
def logica_inibidor(saidaA,saidaB):
    ...
    try:
        ...
        command = ("7000 185920E13300006")  # Envia "abriu portão da garagem" para o moni
        s.send(str.encode(command))
        reply = s.recv(1024)
        print(reply.decode('utf-8'))
...
##    tempo_inibido = 300 # fica aberto 30 segundos com o inibidor
##    verificar = 0
##    stop30 = 0
##    alguem_passou = 0
...
##        if garagem_aberto == 1:
##    
##            print("\nO portão da garagem já fechou após inibidor!")
##
##            try:
##
##                signal.alarm(2)
...
##                s.connect ((host,port))
##
##                command = ("7000 185920R13300006")  # Envia Fechou portão da garagem para o moni
##                s.send(str.encode(command))
##                reply = s.recv(1024)
##                print(reply.decode('utf-8'))
##                signal.alarm(0)
...
##    if garagem_aberto == 0: # Se o portão ainda esta aberto

    fecha_portao(saidaA,saidaB)

    vrg = 0

    print("Saiu da logica inibidor")
...
def fecha_portao(saidaA,saidaB):
    ...
    def dupla_passagem():
        ...
        try:
            ...
            s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
            s.connect ((host,port))
            command = ("7000 185920E13000021")  # Envia Dupla passagem para o moni
            s.send(str.encode(command))
            reply = s.recv(1024)
            print(reply.decode('utf-8'))
    ...
    def violacao_pedestre():
        ...
        try:
            ...
            s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
            s.connect ((host,port))
            command = ("7000 185920E13000006")  # Envia vioação garagem para o moni
            s.send(str.encode(command))
            reply = s.recv(1024)
            print(reply.decode('utf-8'))
...
################################################### COMEÇA A FECHAR ##################################
    ...
    tempo = 200 # Tempo que leva pra fechar = 12 seg.
    stop20 = 0
    stop21 = 0
    carro_na_calcada = 0
    ...
    while tempo > 0:
        ...
        if garagem_aberto == 1 and tempo < 80: # SE O PM JA FECHOU E PASSARAM PELO MENOS 12 SEGUNDOS (tempo que o portão demora pra fechar)
...
def logica_garagem(saidaA, saidaB):
    ...
    while (t > 0):
        ...
        if (barreira_2 == 0): # Alguém acionou a barreira 2 ?
            ...
            print ("Barreira 2 acionou,Verificando...")
                ...
                if (barreira_1 == 1 and opB == 1 and op2 == 0 and op3 == 0): #Se b1 não está acionada e não há nenhum veiculo entrando
                    ...
                    if(barreira_2 == 1): # Se a barreira 2 desacionou sem acionar a barreira 1
                        ...
                        try:
                            ...
                            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                            s.connect ((host,port))
                            command = ("7000 185920E13000006")  # Violação portão da garagem
                            s.send(str.encode(command))
                            reply = s.recv(1024)
                            print(reply.decode('utf-8'))
                ...
                if (barreira_1 == 0 and opA == 1 and op2 == 0 and aguardando_sair == 0): # Se b1  e op1 (entrando um veiculo) forem verdadeiros
                    ...
                    while aguardando_sair == 1:
                        ...
                        if(barreira_2 == 1): # Se a barreira 2 desacionou
                            ...
                            print("Veiculo ja entrou, fechando...")
...
def thread_violacao(saidaA,saidaB,vrg):
    ...
    ##             
                    ...
                    if(garagem_aberto == 0 and vrg == 0):
                        ...
                        print("Abriu sem acionamento e ainda não fechou")
    ...
    ##                    bus.write_byte_data(MCP23017,0x014,saidaA)
                        ...
                        arquivo.write(" ")
                        arquivo.write(hs)
                        arquivo.write(" Evento: Garagem abriu sem acionamento")
                        arquivo.write("\n")
                        arquivo.close()
...
##        bus.write_byte_data(MCP23017,0x015,saidaB)
        ...
        try:
            ...
            command = ("7000 185920E13000017")  # Envia RESET CMM
            s.send(str.encode(command))
            reply = s.recv(1024)
            print(reply.decode('utf-8'))
...
#########################################################################################
    ...
    if garagem_aberto == 0 and vrg == 0:
        ...
        print("Violação, Portão abriu sem acionamento")
        ...
        saidaA = saidaA - 0b10000000 #deslga rele 8 (Giroflex)
        bus.write_byte_data(MCP23017,0x014,saidaA)
    ...
    if (mud == 1): # Chave tanto pra modo de manutenção como mudança(pulso ou retenção da chave)
        ...
        if (mud == 1): # Se a chave permanecer ligada, entra em modo de mudança
            ...
            try:
                ...
                command = ("7000 185920E13000020")  # Envia Chave de mudança acionada (verificar evento e setor)
                s.send(str.encode(command))
                reply = s.recv(1024)
                print(reply.decode('utf-8'))
    ...
    if (falha_energia_nobreak == 1 and falha_nobreak == 0): # entrada IN8
        ...
        if (falha_energia_nobreak == 1):
            ...
            try:
                ...
                command = ("7000 185920E30100019")  # Envia falha na alimenação do noobreak
                s.send(str.encode(command))
                reply = s.recv(1024)
                print(reply.decode('utf-8'))
...
##
    if (falha_energia_nobreak == 0 and falha_nobreak == 1): # entrada IN8
        ...
        try:
            ...
            command = ("7000 185920R30100019")  # Envia Restauração alimenação do noobreak
            s.send(str.encode(command))
            reply = s.recv(1024)
            print(reply.decode('utf-8'))
