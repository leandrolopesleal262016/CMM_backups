...
host_servidor = '172.55.0.17'  # Host servidor SEA (PHP e moni como clientes)
...
def modo_manutencao(hs,data,teste_manutencao):
    ...
    try:
        ...
        command = ("7000 185734E60700008")  # Modo teste de funcionamento
        s.send(str.encode(command))
        reply = s.recv(1024)
        print(reply.decode('utf-8'))
    ...
    while(modo_manutencao(hs,data,teste_manutencao)== 1):
        ...
        if manut == 1:
            ...
            pygame.mixer.music.load('mp3/124.mp3') # Fazendo varredura das entradas
            pygame.mixer.music.play()
            while pygame.mixer.music.get_busy():
                time.sleep(0.1)
            ...
            try:
                ...
                command = ("7000 185734R60700008")  # Modo teste de funcionamento
                s.send(str.encode(command))
                reply = s.recv(1024)
                print(reply.decode('utf-8'))
                ...
                print("Data: ",data, "Hora: ",hs,"Evento: Não enviou o evento Restauração Modo teste de funcionamento")
                s.close()
            ...
            arquivo.write(" ")
            arquivo.write(hs)
            arquivo.write(" Evento: Restauração Sistema em modo de manutenção")
            arquivo.write("\n")
            arquivo.close()
...
def intertravamento(saidaA,saidaB,hs,data,php):
    ...
    if(True):
        ...
        if(php == "AP01"):
            ...
            print ("Abertura portão social por acionamento SEA Software")
...
##            arquivo.close()
        ...
        if (php == "AP02"):
            ...
            print("Abriu portão Interno acionamento do SEA Software")
...
##                s.connect ((host,port))
##
##                command = ("7000 185734E13300003")  # Envia abriu portão ECLUSA para a central de monitormento
##                s.send(str.encode(command))
##
##                reply = s.recv(1024)
##                print(reply.decode('utf-8'))
...
##                s.connect ((host,port))
##
##                command = ("7000 185734R13300003")  # Envia fechou portão ECLUSA para a central de monitormento
##                s.send(str.encode(command))
##
##                reply = s.recv(1024)
##                print(reply.decode('utf-8'))
...
##                    time.sleep(0.1)
                ...
                try:
                    ...
                    command = ("7000 185734R13300001")  # Envia fechou portão para a central de monitormento
                    s.send(str.encode(command))
        ...
        if (bt2 == 1):  # Abre portão eclusa com botão para sair
