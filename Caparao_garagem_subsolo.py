...
##        s.connect ((host,port))
##
##        command = ("7000 185922E60700006")  # Modo teste de funcionamento
##        s.send(str.encode(command))
##        reply = s.recv(1024)
##        print(reply.decode('utf-8'))
##        
...
##                s.connect ((host,port))
##
##                command = ("7000 185922R60700006")  # Modo teste de funcionamento
##                s.send(str.encode(command))
##                reply = s.recv(1024)
##                print(reply.decode('utf-8'))
##                
...
def fecha_portao(saidaA,saidaB):
    ...
    def violacao_pedestre():
        ...
        try:
            ...
            s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
            s.connect ((host,port))
            command = ("7000 185922E13000006")  # Envia vioação garagem para o moni
            s.send(str.encode(command))
            reply = s.recv(1024)
            print(reply.decode('utf-8'))
            signal.alarm(0)
...
##        print("Cooler ligado")
        ...
        try:
            ...
            command = ("7000 185922E13000017")  # Envia RESET CMM
            s.send(str.encode(command))
            reply = s.recv(1024)
            print(reply.decode('utf-8'))
            signal.alarm(0)
...
#########################################################################################
    ...
    if(abre_garagem == 1 and vrg == 0 ):    # Sinal de abertura do portão da garagem (TX, MONI ou SEA SOFTWARE)
        ...
        # Quando estiver monitorando o subsolo chamar a logica da garagem
